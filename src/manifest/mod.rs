...
impl CrateData {
    ...
    fn check_crate_type(&self, emscripten: bool) -> Result<()> {
        if emscripten {
            // rustc links emscripten `bin` targets as main modules (`.js` +
            // `.wasm` via emcc); a `cdylib` would be linked as an emscripten
            // *side module* (`-sSIDE_MODULE=2`), which is a different, dlopen
            // style artifact entirely.
            self.bin_name().map(|_| ()).map_err(|_| {
                anyhow!(
                    "Targeting wasm32-unknown-emscripten requires a binary crate, since rustc \
                     drives emcc as the linker for the final executable. Add a `src/main.rs` \
                     with an (optionally empty) `fn main() {{}}` alongside your #[wasm_bindgen] \
                     exports."
                )
            })
        } else {
    ...
    /// The name of this crate's sole binary target, as required by the
    /// emscripten flow.
    pub fn bin_name(&self) -> Result<String> {
        let pkg = &self.data.packages[self.current_idx];
        let mut bins = pkg
            .targets
            .iter()
            .filter(|target| target.kind.iter().any(|k| k == &TargetKind::Bin));
        let bin = bins
            .next()
            .ok_or_else(|| anyhow!("no binary target found in {}", pkg.name))?;
        if bins.next().is_some() {
            bail!(
                "multiple binary targets found in {}; the emscripten target requires exactly one",
                pkg.name
            );
        }
        Ok(bin.name.clone())
    ...
    ///
    /// `emscripten_files` carries the `(js, wasm)` pkg file names when
    /// building for the emscripten target, whose output layout differs from
    /// the wasm-bindgen conventions.
    pub fn write_package_json(
        ...
        disable_dts: bool,
        target: Target,
        emscripten_files: Option<(String, String)>,
    ) -> Result<()> {
        ...
        let npm_data = if let Some((js_file, wasm_file)) = emscripten_files {
            // Deno historically skips package.json — keep that for parity.
            if matches!(target, Target::Deno) {
                return Ok(());
            }
            self.to_emscripten(scope, existing_deps, out_dir, js_file, wasm_file)
        } else {
            match target {
                ...
                Target::Web | Target::Module => {
                    self.to_web(scope, disable_dts, existing_deps, out_dir)
                }
    ...
    /// Package manifest for the emscripten flow.
    ///
    /// emcc emits a self-initializing ES module plus a `.wasm` it references
    /// by filename; there is no `_bg.wasm`, no snippets, and (for now) no
    /// `.d.ts`.
    fn to_emscripten(
        &self,
        scope: &Option<String>,
