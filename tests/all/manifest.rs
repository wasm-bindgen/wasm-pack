...
fn it_creates_a_package_json_default_path() {
    ...
    assert!(crate_data
        .write_package_json(&out_dir, &None, false, Target::Bundler, None)
        .is_ok());
...
fn it_creates_a_pkg_json_with_correct_files_on_node() {
    ...
    wasm_pack::command::utils::create_pkg_dir(&out_dir, false).unwrap();
    assert!(crate_data
        .write_package_json(&out_dir, &None, false, Target::Nodejs, None)
        .is_ok());
...
fn it_creates_a_pkg_json_with_correct_files_on_nomodules() {
    ...
    wasm_pack::command::utils::create_pkg_dir(&out_dir, false).unwrap();
    assert!(crate_data
        .write_package_json(&out_dir, &None, false, Target::NoModules, None)
        .is_ok());
    let package_json_path = &out_dir.join("package.json");
    fs::metadata(package_json_path).unwrap();
    utils::manifest::read_package_json(&fixture.path, &out_dir).unwrap();
...
fn it_creates_a_package_json_with_correct_keys_when_types_are_skipped() {
    ...
    wasm_pack::command::utils::create_pkg_dir(&out_dir, false).unwrap();
    assert!(crate_data
        .write_package_json(&out_dir, &None, true, Target::Bundler, None)
        .is_ok());
    let package_json_path = &out_dir.join("package.json");
    fs::metadata(package_json_path).unwrap();
    utils::manifest::read_package_json(&fixture.path, &out_dir).unwrap();
